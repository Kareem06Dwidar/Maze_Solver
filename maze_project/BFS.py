...
# Function for Breadth-First Search (BFS) algorithm
#queue FIFO
def bfs(maze):
    # Initialization
    start = maze.start  # The starting point of the maze.
    frontier = [start]  # A list that acts as a queue to store cells to be explored.
    explored = [start]  # A list to keep track of cells that have been explored.
    bfsPath = {}  # A dictionary to store the parent-child relationship for each explored cell.

    # Explore the maze using Breadth-First Search (BFS)
    while len(frontier) > 0:
        currCell = frontier.pop(0)  # Get the current cell from the front of the queue.
        ...
        for d in 'ESNW':
            if maze.is_valid_move(currCell, d):
                ...
                # Add the child cell to the frontier for further exploration.
...
#class MazeSolverGUI:
    def __init__(self, master):
        self.master = master
        self.master.title("Maze Solver (BFS)")
    ...
    # Method to solve the maze using BFS and highlight the path
    def solve_maze(self):
        solution_path = bfs(self.maze)
